...
[[nodiscard]] bool valid_towel(int64_t index, const std::string &towel,
                               const std::unordered_set<std::string> patterns,
                               const int64_t max_pattern) noexcept {
  if (index == towel.size()) {
    return true;
  }
  ...
  for (int64_t size = 1; size <= max_pattern && index + size <= towel.size();
    ...
    // NOTE: Can be optimized with a memo
    if (valid_towel(index + size, towel, patterns, max_pattern)) {
      return true;
    }
  }
...
void part1(const std::unordered_set<std::string> &patterns,
  ...
  for (const auto &t : towels) {
    count += valid_towel(0, t, patterns, max_pattern);
  }
